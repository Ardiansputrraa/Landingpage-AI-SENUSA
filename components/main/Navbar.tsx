...
const sections = ['About', 'AI-Model', 'Features', 'Acknowledgment', 'Publications', 'Our-Team'];
...
const Navbar = () => {
  ...
  return (
    <div className="fixed top-7 left-0 w-full flex justify-center z-50 px-4">
      <button className="md:hidden absolute left-4 top-2 z-50 text-white text-2xl" onClick={() => setIsOpen(true)}>
        <FiMenu />
      </button>
        ...
        {sections.map((item, index) => (
          <motion.a
            ...
            className={`relative text-sm md:text-base font-semibold transition-all px-6 py-3 ${
              activeSection === item
                ? 'bg-gradient-to-r from-cyan-500 to-teal-500 text-white font-bold rounded-full' 
      ...
      <motion.div
        ...
        className="fixed top-0 left-0 w-64 h-full bg-gray-900 text-white z-50 flex flex-col items-start p-6 shadow-lg"
      >
